...
FETCH_TAGS := information=office \
 amenity=social_facility amenity=fast_food amenity=childcare amenity=college amenity=dancing_school amenity=dive_centre amenity=dojo amenity=kindergarten amenity=language_school amenity=library amenity=music_school \
 ...
 amenity=studio amenity=coworking_space amenity=restaurant amenity=cafe amenity=ice_cream amenity=bar amenity=pub amenity=biergarten amenity=nightclub amenity=stripclub \
 ...
 leisure=amusement_arcade leisure=adult_gaming_centre leisure=tanning_salon leisure=dance leisure=escape_game leisure=indoor_play leisure=fitness_centre \
...
stats:
	...
	@echo "TODO     : `sed -ne '/TODO/,$$s/^\([a-z.]\)/\1/p' keys.txt  | wc -l` more need categorising"
